...
gulp.task('template:update', async function () {
  return asyncCmd(`git checkout master && git push origin && git checkout template && git fetch upstream && git pull upstream && git checkout master && git pull && git merge template -m "Merged from template." && git push origin`)
  .then(data => {
    console.log(data)
  })
  .catch(err => {
